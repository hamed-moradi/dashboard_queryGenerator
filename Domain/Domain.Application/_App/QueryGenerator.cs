...
{
    public static class QueryGenerator<T> where T : IBaseEntity
    {
        public static (string Query, DynamicParameters Parameters) Select(T model, bool additionalInfo = true)
        {
            var index = 0;
        ...
        public static (string Query, DynamicParameters Parameters) Single(T model, bool additionalInfo = true)
        {
            var index = 0;
            var columns = string.Empty;
            var joins = string.Empty;
        ...
        public static (string Query, DynamicParameters Parameters) GetById(int id, bool additionalInfo = true)
        {
            ...
            var tableName = typeof(T).Name;
            var hasStatus = false;
            var entityField = typeof(T).GetProperties().Where(prop => !Attribute.IsDefined(prop, typeof(HelperField)));
            var relatedFields = typeof(T).GetProperties().Where(prop => Attribute.IsDefined(prop, typeof(RelatedField))).ToList();
            foreach (var item in entityField)
            {
                index++;
                var entity = string.Empty;
        ...
        public static (string Query, DynamicParameters Parameters) Insert(T model)
        {
        ...
        public static (string Query, DynamicParameters Parameters) BulkInsert(IList<T> model)
        {
            if (!model.Any())
            {
        ...
        public static (string Query, DynamicParameters Parameters) Update(T model)
        {
            ...
            query = AdditionalCondition(typeof(T).Name, query);
        ...
        public static (string Query, DynamicParameters Parameters) UpdateByKey(T model)
        {
            var query = $"Update [{typeof(T).Name}] Set ";
            var myProviderInfo = model.GetType().GetProperties().Where(prop => Attribute.IsDefined(prop, typeof(UpdateField)) || Attribute.IsDefined(prop, typeof(UpdateMandatoryField)));
            var parameters = new DynamicParameters();
        ...
        public static (string Query, DynamicParameters Parameters) LogicalDelete(long[] ids)
        {
            ...
            var parameters = new DynamicParameters();
            var query = $"Update [{typeof(T).Name}] Set {columns} WHERE [{nameof(Entity.Id)}] In (";
        ...
        public static (string Query, DynamicParameters Parameters) PhysicalDelete(object[] ids)
        {
            ...
            var query = $"DELETE FROM [{typeof(T).Name}] WHERE [{nameof(Entity.Id)}] In ({flags})";
