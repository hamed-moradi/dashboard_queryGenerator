...
function FileSelected(file){
   ...
   */
    ...
    var elem = $(window.parent.document).find('#' + fieldId);
    if (elem.is('input:text')) {
        elem.val(file.fullPath);
        $(window.parent.document).find("img[data-id='" + fieldId + "']").attr('src', file.fullPath);
    }
    else {
        elem.attr('src', file.fullPath);
    }

    window.parent.closeCustomRoxy(fieldId);
}
